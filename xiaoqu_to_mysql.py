...
def getXiaoquInfo():
    ...
    for row in rows:
        xiaoqu_info_dict[row.district + '_' + row.area + '_' +
                         row.xiaoqu] = row.id
...
if __name__ == '__main__':
    ...
    csv_dir = "{0}/{1}/xiaoqu/{2}/{3}".format(DATA_PATH, SPIDER_NAME, city,
                                              date)
    files = list()
    ...
    for csv in files:
        ...
        print('[%s,%s,%s]新增了:%s行' % (city_ch, district, area,
                                     len(xiaoqu_price_items)))
